...
def _build_impl_cfg(proto, rt_cfg: MegatronLiteConfig):
    ...
    return proto.ImplConfig(**{k: v for k, v in impl_cfg_kwargs.items() if k in init_fields})
...
def _infer_pipeline_tensor_shape(batch: Any, model_cfg: Any, ps) -> tuple[int, int, int]:
    ...
    if tp_size > 1:
        ...
        # Megatron Lite scatters embeddings into Megatron sequence-parallel
        # form before the first layer, so PP activations carry S / (CP * TP).
...
def _cpu_forward_step(model: torch.nn.Module, batch: Any) -> dict:
    inputs = getattr(batch, "inputs", None)
    targets = getattr(batch, "targets", None)
    if inputs is None:
        inputs = getattr(batch, "input_ids", None)
    if targets is None:
        targets = getattr(batch, "labels", None)
    if inputs is None or targets is None:
        raise ValueError("CPU toy runtime requires inputs/targets or input_ids/labels.")
    outputs = model(inputs)
    return {"loss": torch.nn.functional.mse_loss(outputs, targets), "logits": outputs}
...
class MegatronLiteRuntime(RuntimeBase):
    ...
    def build_model(self, hf_path: str | None = None, cfg: MegatronLiteConfig | dict[str, Any] | None = None, **kwargs) -> ModelHandle:
        ...
        if rt_cfg.device == "cpu":
            return self._build_cpu_model(rt_cfg)
    ...
    def _build_cpu_model(self, rt_cfg: MegatronLiteConfig) -> ModelHandle:
        proto = self._load_protocol(rt_cfg)
        impl_cfg = _build_impl_cfg(proto, rt_cfg)
        model_cfg = proto.build_model_config(rt_cfg.hf_path)
        bundle = proto.build_model(model_cfg, impl_cfg=impl_cfg)
        model = bundle.chunks[0] if len(bundle.chunks) == 1 else bundle.chunks
        optimizer = bundle.optimizer
        if optimizer is None and rt_cfg.optimizer is not None:
            opt_name = rt_cfg.optimizer.optimizer.lower()
            if opt_name != "adam":
                raise ValueError(f"CPU toy runtime only supports adam optimizer, got {opt_name!r}.")
            optimizer = torch.optim.AdamW(
                model.parameters(),
                lr=rt_cfg.optimizer.lr,
                weight_decay=rt_cfg.optimizer.weight_decay,
            )
        return ModelHandle(
            model=model,
            optimizer=optimizer,
            lr_scheduler=None,
            parallel_state=SimpleNamespace(pp_size=1, tp_rank=0, cp_rank=0, pp_rank=0),
            config=rt_cfg,
            _extras={
                "model_chunks": bundle.chunks,
                "model_cfg": model_cfg,
                "forward_step": bundle.forward_step or _cpu_forward_step,
                "protocol": proto,
                **bundle.extras,
            },
        )
    ...
    def forward_backward(
        self, handle: ModelHandle, data: Any, loss_fn: Callable | None = None,
    ...
    def optimizer_step(self, handle: ModelHandle) -> tuple[bool, float, int]:
        if handle._optimizer is None:
            return True, 0.0, 0
        step_result = handle._optimizer.step()
        if isinstance(step_result, tuple) and len(step_result) == 3:
            update_successful, grad_norm, num_zeros = step_result
            return update_successful, float(grad_norm), num_zeros
        grad_norm_sq = 0.0
        num_zeros = 0
        for group in getattr(handle._optimizer, "param_groups", []):
        ...
        return True, grad_norm_sq ** 0.5, num_zeros
    ...
    def lr_scheduler_step(self, handle: ModelHandle) -> float | list[float]:
        ...
        if handle._optimizer is not None and getattr(handle._optimizer, "param_groups", None):
            return float(handle._optimizer.param_groups[0].get("lr", 0.0))
        return 0.0
