...
def _router_and_parallel_state(monkeypatch, _megatron_core_unit_stubs):
...
def test_topk_router_returns_finite_scores_and_valid_expert_indices(
    monkeypatch, megatron_core_unit_stubs
):
    TopKRouter, ParallelState = _router_and_parallel_state(monkeypatch, megatron_core_unit_stubs)
...
def test_topk_router_does_not_attach_aux_scaler_in_eval(monkeypatch, megatron_core_unit_stubs):
    TopKRouter, ParallelState = _router_and_parallel_state(monkeypatch, megatron_core_unit_stubs)
    config = _router_config()
...
def test_topk_router_aux_loss_contributes_gate_gradient(monkeypatch, megatron_core_unit_stubs):
    TopKRouter, ParallelState = _router_and_parallel_state(monkeypatch, megatron_core_unit_stubs)
    config = _router_config()
    router = TopKRouter(config, ParallelState(), compute_aux_loss=True)
...
def test_mrope_interleaves_text_height_and_width_sections(megatron_core_unit_stubs):
    _ = megatron_core_unit_stubs
