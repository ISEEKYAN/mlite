...
def test_verl_sft_script_maps_offload_env_to_backend_args(tmp_path):
    ...
    assert "+engine.impl_cfg.optimizer=mc" in command
...
def test_verl_sft_script_maps_fsdp2_optimizer_backend(tmp_path):
    script = (
        Path(__file__).resolve().parents[3]
        / "examples"
        / "verl"
    ...
    command = _run_verl_sft_dry_run(
        script,
        tmp_path,
        MLITE_OPTIMIZER_BACKEND="fsdp2",
        OPTIMIZER_OFFLOAD="False",
    )

    assert "+engine.impl_cfg.optimizer=fsdp2" in command
