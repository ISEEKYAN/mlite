...
- The examples directory is also added to `PYTHONPATH` and loads a local
  compatibility hook for known VERL/Transformers dependency gaps.
...
- `MLITE_OPTIMIZER_BACKEND=distopt` or `fsdp2`
- `MLITE_MODEL_NAME=auto`, `MLITE_IMPL=lite`
- `ATTENTION_BACKEND=flash`
- `DRY_RUN=1` to print the resolved `torchrun` command without launching

The launcher defaults to `MLITE_OPTIMIZER_BACKEND=distopt`, which maps to
Megatron-Core DDP plus distributed optimizer through
`engine.impl_cfg.optimizer=mc`. Set `MLITE_OPTIMIZER_BACKEND=fsdp2` to exercise
the FSDP2 primitive path instead.
